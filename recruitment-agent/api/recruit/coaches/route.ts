...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter by contacted status if campaignId provided
    if (campaignId) {
      const { data: contactedIds, error: contactedError } = await supabaseAdmin
        .from('messages')
        .select('coach_id')
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!first_name || !last_name || !email || !school) {
      return NextResponse.json(
        { error: 'first_name, last_name, email, and school are required' },
        { status: 400 }
      );
    }
    ...
    if (existing) {
      return NextResponse.json(
        { error: 'Coach with this email already exists', coachId: existing.id },
        { status: 409 }
      );
    }
    ...
    if (insertError || !coach) {
      return NextResponse.json(
        { error: insertError?.message || 'Failed to create coach' },
        { status: 500 }
      );
    }
