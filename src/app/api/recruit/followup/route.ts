...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!campaignId || !athleteId) {
      return NextResponse.json({ error: 'campaignId and athleteId are required' }, { status: 400 })
    }
    ...
    const responded = (responses || []).map((r: any) => {
      ...
      return {
        responseId: r.id, coachId: r.coach_id,
        coachName: coach ? `${coach.first_name} ${coach.last_name}` : 'Unknown',
        school: coach?.school || '', division: coach?.division || '',
        sentiment: r.sentiment || 'neutral', responseBody: r.body,
        responseSubject: r.subject, respondedAt: r.created_at,
    ...
    const nonResponded = sentCoachIds
      ...
      .map(id => {
        ...
        return {
          ...
          coachName: coach ? `${coach.first_name} ${coach.last_name}` : 'Unknown',
          school: coach?.school || '', division: coach?.division || '',
          email: coach?.email || '', sentAt: msg?.sent_at,
          daysSince: msg?.sent_at ? Math.floor((Date.now() - new Date(msg.sent_at).getTime()) / (1000 * 60 * 60 * 24)) : null,
    ...
    const followUpTemplates = {
      ...
      responded_neutral: `Coach {{coach_last}},\n\nThank you for getting back to me. I understand you have many athletes to evaluate.\n\nI wanted to follow up and share that I am still very interested in {{school}}.\n\nHere is my updated highlight film:\n{{highlight_url}}\n\nI would love the opportunity to speak with you or visit campus.\n\nRespectfully,\n{{athlete_first}} {{athlete_last}}\n{{high_school}} — Class of {{grad_year}}`,
      no_response: `Coach {{coach_last}},\n\nI hope this message finds you well. I reached out a few weeks ago about my interest in {{school}} and your basketball program.\n\nI am a {{grad_year}} {{position}} from {{high_school}} in {{city}}, {{state}} — {{height}}, averaging {{ppg}} PPG, {{rpg}} RPG, and {{spg}} SPG.\n\nHere is my highlight film:\n{{highlight_url}}\n\nPlease feel free to contact me at {{athlete_email}} or my father, {{parent_name}}, at {{parent_email}}.\n\nThank you for your time, Coach.\n\nRespectfully,\n{{athlete_first}} {{athlete_last}}\n{{high_school}} — Class of {{grad_year}}`,
    }

    return NextResponse.json({
      success: true, responded, nonResponded, followUpTemplates,
      stats: {
        totalSent: sentCoachIds.length, responded: responded.length,
        nonResponded: nonResponded.length, followUpsSent: followedUpCoachIds.size,
        responseRate: sentCoachIds.length > 0 ? Math.round((responded.length / sentCoachIds.length) * 100) : 0,
      },
    })
  } catch (err: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!campaignId || !athleteId || !coachId || !message) {
      return NextResponse.json({ error: 'campaignId, athleteId, coachId, and message are required' }, { status: 400 })
    }

    const { data: athlete } = await supabaseAdmin.from('athletes').select('*').eq('id', athleteId).single()
    if (!athlete) return NextResponse.json({ error: 'Athlete not found' }, { status: 404 })

    const { data: coach } = await supabaseAdmin.from('coaches').select('*').eq('id', coachId).single()
    ...
    const { data: msgRecord, error: insertError } = await supabaseAdmin
      .from('messages')
      .insert({
        campaign_id: campaignId, coach_id: coachId, athlete_id: athleteId,
        type: 'follow_up', channel: 'resend', to_address: coach.email,
        subject: renderedSubject, body: renderedBody, status: 'queued',
      })
      .select()
      .single()
    ...
    try {
      ...
      return NextResponse.json({ success: true, messageId: msgRecord.id, resendId: result.data?.id, sentTo: coach.email })
    } catch (sendErr: any) {
      await supabaseAdmin
        .from('messages')
      ...
      return NextResponse.json({ error: sendErr.message, messageId: msgRecord.id }, { status: 500 })
    }
  } catch (err: any) {
